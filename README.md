...
```dart
// enum VmCoinInstance { freespin, freeRace, freebet, casinoBonus, cashFcfa, cashEuros }
final coin = VmCoinViewModel.fromInstance(file, VmCoinInstance.cashEuros);
coin.bind(controller.stateMachine);

// Or the default instance:
final defaultCoin = VmCoinViewModel.fromDefaultInstance(file);
```
