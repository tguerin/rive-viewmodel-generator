...
- Nested `viewModel`-typed properties now resolve to their real top-level class
  ...
  after the property. A view model embedded in another (e.g. a coin used as a
  library) now returns the shared class — with its instance enum and factories —
  and the singular/plural property-name mismatches the old name heuristic missed
  are handled too.
